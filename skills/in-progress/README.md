...
- **[spawn](./spawn/SKILL.md)** — Hand the current conversation off to a fresh background agent that picks up the work immediately, seeded with a handoff summary via the bundled `spawn.sh`; agent-agnostic — claude, codex, pi, cursor, opencode, or copilot. User-invoked.
